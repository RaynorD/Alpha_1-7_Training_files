class CfgSettings {
    ...
    isMissionType               =   0;      // 0: Custom, 1: Operation, 2: Training (Default: 1)
    aiSystemDifficulty          =   0;      // 0: Standard, 1: Desert, 2: Dumb As Fuck (Default: 0)
    use7cavZeusModules          =   0;      // 0 or 1 Allow the misson to use the 7Cav module system. (Default: 1)

    // Hints and documents
    useStartHint                =   1;      // 0 or 1 Allow the mission to run the RedLightHint or TrainingMissionHint depends on mission type (Default: 1)
    setRedLightTime             =   5;     // Seconds for how long it is red light (Default: 60)
